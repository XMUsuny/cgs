...
cl::opt<std::string> UncoveredUpdateInterval(
    ...
    cl::cat(StatsCat));
/*
cl::opt<std::string> targetBranchUpdateInterval(
    "target-branch-update-interval", cl::init("300s"),
    cl::desc("Update interval for uncovered instructions (default=300s)"),
    cl::cat(StatsCat));
*/
...
StatsTracker::StatsTracker(Executor &_executor, std::string _objectFilename,
  ...
  /*
  if (userSearcherRequiresDGS()) {
    executor.timers.add(std::make_unique<Timer>(time::Span{targetBranchUpdateInterval}, [&]{
      executor.updateTargetBranch = true;
    }));
  }*/
...
void StatsTracker::writeStatsHeader() {
  ...
  create << "CREATE TABLE stats ("
             ...
             << "PartlyCoveredBranches INTEGER,"
             << "FullyCoveredBranches INTEGER,"
             << "totalConstrants INTEGER,"
             << "symbolicConstrants INTEGER,"
             << "concreteConstrants INTEGER,"
             << "fullyCoveredSymbolicConstrants INTEGER,"
             << "fullyCoveredConcreteConstrants INTEGER"
         << ')';
  ...
  insert << "INSERT OR FAIL INTO stats ("
             ...
             << "QueryCexCacheHits,"
             << "ArrayHashTime,"
             << "PartlyCoveredBranches,"
             << "FullyCoveredBranches,"
             << "totalConstrants,"
             << "symbolicConstrants,"
             << "concreteConstrants,"
             << "fullyCoveredSymbolicConstrants,"
             << "fullyCoveredConcreteConstrants" 
         << ") VALUES ("
             << "?,"
             << "?,"
             << "?,"
...
#endif
  sqlite3_bind_int64(insertStmt, 21, executor.partlyCoveredBranches.size());
  sqlite3_bind_int64(insertStmt, 22, executor.fullyCoveredBranches.size());

  sqlite3_bind_int64(insertStmt, 23, executor.totalConstrants.size());
  sqlite3_bind_int64(insertStmt, 24, executor.symbolicConstrants.size());
  sqlite3_bind_int64(insertStmt, 25, executor.concreteConstrants.size());
  sqlite3_bind_int64(insertStmt, 26, executor.fullyCoveredSymbolicConstrants.size());
  sqlite3_bind_int64(insertStmt, 27, executor.fullyCoveredConcreteConstrants.size());
